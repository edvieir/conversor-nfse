...
def consultar_chave_avulsa(
    ...
    try:
        ...
        if cstat == "138":
            ...
            return None, (
                "NF-e não distribuída para esse CNPJ no Ambiente Nacional (cStat=137).\n\n"
                "O que significa:\n"
                "• A SEFAZ Nacional reconhece a nota, mas não a distribuiu para esse CNPJ ainda\n"
                "• Isso ocorre em NF-es autorizadas pelo SVRS (SEFAZ Virtual do RS que processa CE) — "
                "o SVRS às vezes demora para replicar o vínculo destinatário→chave no Ambiente Nacional\n\n"
                "Solução:\n"
                "• Use a aba Consulta em Lote — o distNSU sincroniza a fila completa e traz essas notas automaticamente"
            )
        else:
            _MENSAGENS = {
