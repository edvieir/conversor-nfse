            ...
            for xml_path in arquivos:
                ...
                try:
                    ...
                    ws.append([
                        ...
                        None,                              # F  Vencimento       (FIXO vazio)
                        None,                              # G  Número RPS       (FIXO vazio)
                        None,                              # H  Série RPS        (FIXO vazio)
                        None,                              # I  Tipo RPS         (FIXO vazio)
                        "Tributação Fora do Município",    # J  Natureza         (FIXO)
                        None,                              # K  Regime Especial  (FIXO vazio)
                        "Não",                             # L  Simples Nacional (FIXO)
                        None,                              # M  Incentivador     (FIXO vazio)
                        item,                              # N  Item da Lista    (XML)
                        _cnae_desc(cnae9),                 # O  CNAE             (XML cnae9+desc)
                        None,                              # P  ART              (FIXO vazio)
                        None,                              # Q  Código Obra      (FIXO vazio)
                        None,                              # R  Nº Empenho       (FIXO vazio)
                        _str(d.get("desc")),               # S  Discriminação    (XML)
                        vS,                                # T  Valor Serviços   (XML float)
                        None,                              # U  Deduções         (FIXO vazio)
                        None,                              # V  Desc. Condic.    (FIXO vazio)
                        None,                              # W  Desc. Incondic.  (FIXO vazio)
                        ret_federais,                      # X  Ret. Federais    (XML soma, 0 se nenhuma)
                        None,                              # Y  Outras Retenções (FIXO vazio)
                        vPIS    if vPIS    else None,      # Z  PIS              (XML, vazio se 0)
                        vCOFINS if vCOFINS else None,      # AA COFINS           (XML, vazio se 0)
                        vIRRF   if vIRRF   else None,      # AB IRRF             (XML vRetIRRF)
                        vCSLL   if vCSLL   else None,      # AC CSLL             (XML vRetCSLL)
                        vINSS   if vINSS   else None,      # AD INSS             (XML, vazio se 0)
                        ...
                        None,                              # AL Inscrição Prest. (FIXO vazio)
